...
<html>
<head>
    <link href="../css/style.css" rel=stylesheet /> <title>Infini</title> 
...
<?php 
    ...
    $errorFirst = $errorLast = $errorLast = $errorPosition = $errorLocation = "";
    ...
    if(isset($_POST["submit"])) {
        ...
        if(empty($_POST["cvn"])) {
            $errorLocation = "<span class='error'>Please enter the CV number</span>";
        }
  
        echo "teaktjslkjadja";
        if($errorFirst == "" && $errorLast == "" &&  $errorLast == "" && $errorPosition == "" && $errorLocation == ""){
            $fName = sanitize($_POST["fName"]);           
            $lName = sanitize($_POST["lName"]);
            $zipcode = sanitize($_POST["zipcode"]);
            $ccNum = sanitize($_POST["ccNum"]);
            $expDate = sanitize($_POST["month"] . "/" . $POST["year"]);
            $cvn = sanitize($_POST["cvn"]);
            
            echo "CALL insert_cc_data('$f_name', '$l_name', '$zipcode', '$ccNum', '$expDate', '$cvn', '$ccNum', '$expDate', '$cvn');";
//mysqli_query($link, "CALL insert_cc_data('$f_name', '$l_name', '$zipcode', '$ccNum', '$expDate', '$cvn', '$ccNum', '$expDate', '$cvn');");
...
?>
        ...
        <style>
            ...
            <form action="index.php" method="post">
                ...
                Billing Zip Code: 
                <input type=text name="zipcode" required pattern="[0-9]{5}" placeholder="Zip Code"><br/>
                
                CC Number: 
                <input type=text name="ccNum" required pattern="[0-9]{16}" placeholder="16 digits, no spaces"><br/>
                ...
                CVN: 
                <input type=text name="cvn" required pattern="[0-9]{3,4}" placeholder="CV Number"><br/>   
